...
const SKILLS: [(&str, &str, &str); 18] = [
    ("acr", "dex", "Acrobatics"), ("ani", "wis", "Animal Handling"), ("arc", "int", "Arcana"),
    ("ath", "str", "Athletics"), ("dec", "cha", "Deception"), ("his", "int", "History"),
    ("ins", "wis", "Insight"), ("itm", "cha", "Intimidation"), ("inv", "int", "Investigation"),
    ("med", "wis", "Medicine"), ("nat", "int", "Nature"), ("prc", "wis", "Perception"),
    ("prf", "cha", "Performance"), ("per", "cha", "Persuasion"), ("rel", "int", "Religion"),
    ("slt", "dex", "Sleight of Hand"), ("ste", "dex", "Stealth"), ("sur", "wis", "Survival"),
];
...
pub fn roll_d20<R: FnMut() -> f64>(
    modifier: i64, advantage: bool, disadvantage: bool, dc: Option<i64>, mut rng: R,
) -> D20Result {
    let d20 = |rng: &mut R| 1 + (rng() * 20.0) as i64 % 20;
    let adv = advantage && !disadvantage;
    let dis = disadvantage && !advantage;
    let rolls = if adv || dis { vec![d20(&mut rng), d20(&mut rng)] } else { vec![d20(&mut rng)] };
    let kept = if adv { *rolls.iter().max().unwrap() }
        else if dis { *rolls.iter().min().unwrap() }
        else { rolls[0] };
    let total = kept + modifier;
    D20Result {
        crit: kept == 20,
        fumble: kept == 1,
        success: dc.map(|d| if kept == 20 { true } else if kept == 1 { false } else { total >= d }),
        rolls, kept, total,
    }
}
...
pub fn format_d20(r: &D20Result, modifier: i64) -> String {
    ...
    let special = if r.crit { " · ⭐ CRITIQUE" } else if r.fumble { " · 💀 ÉCHEC CRITIQUE" } else { "" };
...
pub async fn run(state: &McpState, name: &str, args: &Value) -> Result<Value> {
    ...
    match name {
        "dnd5e_roll_check" => {
            ...
            let check = str_arg(args, "check").ok_or_else(|| anyhow!("'check' is required"))?
                ...
                .find(|(k, _, l)| *k == check || l.to_lowercase() == check)
            {
                ...
                (label.to_string(), ability_modifier(score(&ability)) + (mult * prof as f64).floor() as i64)
            } else {
                bail!(
                    "Unknown check '{check}'. Abilities: {} (+_save) · skills: {}",
                    ABILITIES.iter().map(|(k, _)| *k).collect::<Vec<_>>().join("/"),
                    SKILLS.iter().map(|(_, _, l)| *l).collect::<Vec<_>>().join(", ")
                );
            };
        ...
        "dnd5e_adjust_stats" => {
            ...
            let hp_max = get_path(&actor, "system.attributes.hp.max").and_then(Value::as_i64).unwrap_or(i64::MAX);
            ...
            for (key, path) in requested {
                ...
                if *key == "hp" { after = after.min(hp_max); }
...
mod tests {
    ...
    fn avantage_et_criticals() {
        ...
        assert_eq!(roll_d20(0, true, true, None, seq(&[0.2, 0.8])).rolls.len(), 1);
