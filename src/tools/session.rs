...
async fn resolve_scene(state: &McpState, args: &Value) -> Result<Value> {
    ...
    let scene = match (str_arg(args, "scene_id"), str_arg(args, "scene_name")) {
        (Some(id), _) => state.foundry.get_document("scenes", Some(&id), None, Some(&fields)).await?,
        (None, Some(n)) => state.foundry.get_document("scenes", None, Some(&n), Some(&fields)).await?,
        _ => {
            let w = json!({"active": true}).as_object().cloned().unwrap();
            state.foundry.get_documents("scenes", Some(&w), Some(&fields), 0, Some(1)).await?.into_iter().next()
        }
    };
    scene.ok_or_else(|| anyhow!("Scene not found (no active scene and none specified)"))
}

async fn find_actor(state: &McpState, args: &Value, key_id: &str, key_name: &str, fields: &[String]) -> Result<Value> {
    let doc = match (str_arg(args, key_id), str_arg(args, key_name)) {
        (Some(i), _) => state.foundry.find_document("actors", &i, Some(fields)).await?,
        (None, Some(n)) => state.foundry.get_document("actors", None, Some(&n), Some(fields)).await?,
...
pub async fn run(state: &McpState, name: &str, args: &Value) -> Result<Value> {
    ...
    match name {
        "show_journal_to_players" => {
            ...
            foundry
                .emit_with_ack("showEntry", &[json!(uuid), json!({"force": force, "users": users})])
                .await?;
        ...
        "share_image" => {
            ...
            if let Some(t) = str_arg(args, "title") { cfg["title"] = json!(t); }
            if let Some(c) = str_arg(args, "caption") { cfg["caption"] = json!(c); }
            foundry.emit("shareImage", &[cfg]).await?;
            Ok(text_response(&json!({"shared": image})))
        }
        "toggle_pause" => {
            let paused = args.get("paused").and_then(Value::as_bool)
                .ok_or_else(|| anyhow!("'paused' is required"))?;
            let user_id = foundry.user_id().await;
            foundry
                .emit("pause", &[json!(paused), json!({"broadcast": true, "userId": user_id})])
                .await?;
        ...
        "activate_scene" | "pull_users_to_scene" => {
            ...
            if name == "activate_scene" {
                foundry
                    .modify_document("Scene", "update", json!({
            ...
            let want_pull = name == "pull_users_to_scene"
                || args.get("pull_users").and_then(Value::as_bool).unwrap_or(false);
            if want_pull {
                pulled = args.get("users").and_then(Value::as_array)
                    .map(|a| a.iter().filter_map(Value::as_str).map(String::from).collect())
                    .unwrap_or_default();
                if pulled.is_empty() {
                    let fields = vec!["_id".into()];
                    let users = foundry.get_documents("users", None, Some(&fields), 0, None).await?;
                ...
                for uid in &pulled {
                    foundry.emit("pullToScene", &[json!(scene_id), json!(uid)]).await?;
                }
            }
        ...
        "list_tokens" | "move_token" | "update_token" => {
            ...
            let empty = vec![];
            let tokens = scene.get("tokens").and_then(Value::as_array).unwrap_or(&empty);
            if name == "list_tokens" {
            ...
            if name == "move_token" {
                for key in ["x", "y", "elevation"] {
                    if let Some(v) = args.get(key) {
                        if !v.is_null() { updates.insert(key.into(), v.clone()); }
                    }
                }
                if updates.is_empty() { bail!("Provide at least one of: x, y, elevation"); }
            } else {
                updates = args.get("updates").and_then(Value::as_object).cloned()
                    .filter(|u| !u.is_empty())
                    .ok_or_else(|| anyhow!("'updates' must not be empty"))?;
            }
            let mut update_doc = Value::Object(updates.clone());
            update_doc["_id"] = token["_id"].clone();
            let result = foundry.modify_document("Token", "update", json!({
                "action": "update", "diff": false, "recursive": true, "render": true,
        ...
        "place_token" => {
            ...
            let x = args.get("x").and_then(Value::as_f64).ok_or_else(|| anyhow!("'x' is required"))?;
            let y = args.get("y").and_then(Value::as_f64).ok_or_else(|| anyhow!("'y' is required"))?;
            ...
            if token.get("name").and_then(Value::as_str).unwrap_or("").is_empty() {
        ...
        "toggle_actor_condition" => {
            let condition = str_arg(args, "condition").ok_or_else(|| anyhow!("'condition' is required"))?;
            let active = args.get("active").and_then(Value::as_bool)
                .ok_or_else(|| anyhow!("'active' is required"))?;
            let status = CORE_STATUS_EFFECTS.iter().find(|(id, _, _)| *id == condition)
                .ok_or_else(|| anyhow!(
                    "Unknown condition '{condition}'. Available: {}",
                    CORE_STATUS_EFFECTS.iter().map(|(i, _, _)| *i).collect::<Vec<_>>().join(", ")
                ))?;
            let fields = vec!["_id".into(), "name".into(), "effects".into()];
            let actor = find_actor(state, args, "_id", "name", &fields).await?;
            let empty = vec![];
            let effects = actor.get("effects").and_then(Value::as_array).unwrap_or(&empty);
            let existing: Vec<&Value> = effects.iter()
                .filter(|e| e.get("statuses").and_then(Value::as_array)
                    .map(|s| s.contains(&json!(condition))).unwrap_or(false))
                .collect();
...
async fn resolve_scene_by_ident(state: &McpState, args: &Value) -> Result<Value> {
    ...
    let doc = match (str_arg(args, "_id"), str_arg(args, "name")) {
        (Some(i), _) => state.foundry.get_document("scenes", Some(&i), None, Some(&fields)).await?,
        (None, Some(n)) => state.foundry.get_document("scenes", None, Some(&n), Some(&fields)).await?,
...
async fn scene_tokens(state: &McpState, scene_id: Option<String>) -> Result<Value> {
    ...
    let scene = match scene_id {
        Some(id) => state.foundry.get_document("scenes", Some(&id), None, Some(&fields)).await?,
        None => {
            let w = json!({"active": true}).as_object().cloned().unwrap();
            state.foundry.get_documents("scenes", Some(&w), Some(&fields), 0, Some(1)).await?.into_iter().next()
        }
...
async fn with_names(state: &McpState, combat: &Value, order: Vec<Value>) -> Result<Vec<Value>> {
    if order.is_empty() || order.iter().all(|c| c.get("name").map_or(false, |n| !n.is_null())) {
        return Ok(order);
    }
    let scene_id = combat.get("scene").and_then(Value::as_str)
        .or_else(|| order.first().and_then(|c| c.get("sceneId")).and_then(Value::as_str))
        .map(String::from);
    let Some(sid) = scene_id else { return Ok(order) };
    let fields = vec!["_id".into(), "tokens".into()];
    let scene = state.foundry.get_document("scenes", Some(&sid), None, Some(&fields)).await?;
    let empty = vec![];
    let tokens = scene.as_ref().and_then(|s| s.get("tokens")).and_then(Value::as_array).unwrap_or(&empty);
    Ok(order.into_iter().map(|mut c| {
        if c.get("name").map_or(true, Value::is_null) {
            let tname = tokens.iter()
                .find(|t| t["_id"] == c["tokenId"])
                .and_then(|t| t.get("name")).cloned().unwrap_or(Value::Null);
            c["name"] = tname;
        }
        c
    }).collect())
...
async fn manage_combat(state: &McpState, args: &Value) -> Result<Value> {
    ...
    let sorted = |combat: &Value| -> Vec<Value> {
        let mut list = combat.get("combatants").and_then(Value::as_array).cloned().unwrap_or_default();
        list.sort_by(|a, b| {
            let ia = a.get("initiative").and_then(Value::as_f64).unwrap_or(f64::NEG_INFINITY);
            let ib = b.get("initiative").and_then(Value::as_f64).unwrap_or(f64::NEG_INFINITY);
    ...
    if action == "create" {
        ...
        let empty = vec![];
        let all = scene.get("tokens").and_then(Value::as_array).unwrap_or(&empty);
        let wanted: Option<Vec<String>> = args.get("tokens").and_then(Value::as_array)
            .map(|a| a.iter().filter_map(Value::as_str).map(String::from).collect());
        let tokens: Vec<&Value> = all.iter().filter(|t| match &wanted {
            Some(w) => w.iter().any(|x| t["_id"] == json!(x) || t["name"] == json!(x)),
            None => t.get("actorId").map_or(false, |a| !a.is_null()),
        }).collect();
        let created = foundry.modify_document("Combat", "create", json!({
            "action": "create", "broadcast": false, "renderSheet": false, "keepId": false,
            "data": [{"scene": scene["_id"], "active": true}],
        })).await?;
        let combat_id = created.pointer("/result/0/_id").and_then(Value::as_str)
            .ok_or_else(|| anyhow!("Combat creation gave no _id: {created}"))?.to_string();
        if !tokens.is_empty() {
            let data: Vec<Value> = tokens.iter().map(|t| json!({
    ...
    let combat = match str_arg(args, "combat_id") {
        Some(id) => foundry.get_document("combats", Some(&id), None, None).await?,
        None => {
            let all = foundry.get_documents("combats", None, None, 0, None).await?;
            all.iter().find(|c| c.get("active") == Some(&json!(true))).cloned()
    ...
    match action.as_str() {
        "add_combatants" => {
            let scene = scene_tokens(state, combat.get("scene").and_then(Value::as_str).map(String::from)).await?;
            let empty = vec![];
            let all = scene.get("tokens").and_then(Value::as_array).unwrap_or(&empty);
            let wanted: Vec<String> = args.get("tokens").and_then(Value::as_array)
                .map(|a| a.iter().filter_map(Value::as_str).map(String::from).collect())
                .filter(|v: &Vec<String>| !v.is_empty())
                .ok_or_else(|| anyhow!("'tokens' is required"))?;
            let tokens: Vec<&Value> = all.iter()
                .filter(|t| wanted.iter().any(|x| t["_id"] == json!(x) || t["name"] == json!(x)))
                .collect();
            if tokens.is_empty() { bail!("none of the tokens were found on the scene"); }
            let data: Vec<Value> = tokens.iter().map(|t| json!({
                "tokenId": t["_id"], "sceneId": scene["_id"], "actorId": t["actorId"],
                "hidden": t.get("hidden").cloned().unwrap_or(json!(false)),
            })).collect();
        ...
        "set_initiative" => {
            let who = str_arg(args, "combatant").ok_or_else(|| anyhow!("'combatant' is required"))?;
            let initiative = args.get("initiative").and_then(Value::as_f64)
                .ok_or_else(|| anyhow!("'initiative' is required"))?;
            let empty = vec![];
            let all = combat.get("combatants").and_then(Value::as_array).unwrap_or(&empty);
            let target = all.iter()
                .find(|c| c["_id"] == json!(who) || c["name"] == json!(who))
                .ok_or_else(|| anyhow!("Combatant not found: {who}"))?;
            let result = foundry.modify_document("Combatant", "update", json!({
                "action": "update", "diff": false, "recursive": true, "render": true,
        ...
        "start" => {
            let result = foundry.modify_document("Combat", "update", json!({
                "action": "update", "diff": false, "recursive": true, "render": true,
                "updates": [{"_id": combat_id, "round": 1, "turn": 0}],
            })).await?;
            Ok(text_response(&json!({"combat": combat_id, "started": true, "result": result})))
        }
        "next_turn" | "next_round" => {
            ...
            let (new_round, new_turn) = if action == "next_round" || (turn + 1) as usize >= order.len() {
                (round + 1, 0u64)
            } else {
                (round, turn + 1)
            };
        ...
        "end" => {
            let result = foundry.modify_document("Combat", "delete", json!({
                "action": "delete", "broadcast": false, "ids": [combat_id],
            })).await?;
            Ok(text_response(&json!({"combat": combat_id, "ended": true, "result": result})))
        }
...
async fn control_playlist(state: &McpState, args: &Value) -> Result<Value> {
    ...
    let action = str_arg(args, "action").ok_or_else(|| anyhow!("'action' is required"))?;
    let playlist = foundry.find_document("playlists", &pl_arg, None).await?
        .ok_or_else(|| anyhow!("Playlist not found: {pl_arg}"))?;
    let mut sounds = playlist.get("sounds").and_then(Value::as_array).cloned().unwrap_or_default();
    ...
    let (playing, update) = if action == "stop" {
        ...
        let playing_ids: Vec<Value> = match str_arg(args, "sound") {
            Some(sarg) => {
                let s = sounds.iter().find(|s| s["_id"] == json!(sarg) || s["name"] == json!(sarg))
            ...
            None => {
                let first = sounds.first().ok_or_else(|| anyhow!("playlist has no sounds"))?;
                vec![first["_id"].clone()]
            }
        };
        let names: Vec<Value> = sounds.iter()
    ...
    let result = foundry.modify_document("Playlist", "update", json!({
        "action": "update", "diff": false, "recursive": true, "render": true,
        "updates": [update_doc],
    })).await?;
    Ok(text_response(&json!({
...
async fn draw_from_table(state: &McpState, args: &Value) -> Result<Value> {
    ...
    let table = foundry.find_document("tables", &t_arg, None).await?
        .ok_or_else(|| anyhow!("RollTable not found: {t_arg}"))?;
    let modifier = args.get("modifier").and_then(Value::as_i64).unwrap_or(0);
    let rolls_n = args.get("rolls").and_then(Value::as_u64).unwrap_or(1).clamp(1, 10);
    ...
    if args.get("post").and_then(Value::as_bool).unwrap_or(true) {
        let body: Vec<String> = draws.iter().map(|d| format!(
            "<p><strong>{}</strong>{} → {}</p>",
            d["roll"],
            if modifier != 0 { format!(" (dont {modifier:+})") } else { String::new() },
        ...
        if let Some(w) = args.get("whisper_users").and_then(Value::as_array) {
            if !w.is_empty() { message["whisper"] = json!(w); }
        }
        foundry.modify_document("ChatMessage", "create", json!({
            "action": "create", "broadcast": false, "renderSheet": false, "keepId": false,
            "data": [message],
        })).await?;
        posted = true;
